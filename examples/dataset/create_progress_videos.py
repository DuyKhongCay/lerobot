...
def _resolve_task_name(row: pd.Series, local_path: Path) -> str:
    ...
    try:
        ...
        if tasks_parquet.exists():
            ...
            _raw_idx = row.get("task_index", 0)
            task_idx = int(_raw_idx) if _raw_idx is not None else 0
...
def load_progress_data(local_path: Path, episode: int) -> np.ndarray | None:
    ...
    if episode_df.empty:
        ...
        return None
    episode_df = episode_df.sort_values(by="frame_index")  # type: ignore
    ...
    return episode_df[["frame_index", str(progress_column)]].rename(columns={str(progress_column): "progress"}).values  # type: ignore
